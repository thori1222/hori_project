...
public:
	...
	ShredMemoryAccessor():m_opened(false), p_shared_data(0){}
	ShredMemoryAccessor(bool serve_host, const char* sharemem_name, const char* sharemutex_name=""):m_opened(false), p_shared_data(0){
...
#endif
	RTCtrlFramework::RtcMutex mutex;

};

template <class T>
bool ShredMemoryAccessor<T>::TryOpen(const char* sharemem_name=NULL, const char* sharemutex_name="")
{
	try{
...
template <class T>
bool ShredMemoryAccessor<T>::Open(bool serve_host, const char* sharemem_name=NULL, const char* sharemutex_name="")
{
...
// おそらく自動解放
template <class T>
ShredMemoryAccessor<T>::~ShredMemoryAccessor()
{
...
template <class T>
bool ShredMemoryAccessor<T>::lock()
{
	return mutex.lock();
}
...
template <class T>
void ShredMemoryAccessor<T>::unlock()
{
	mutex.unlock();
}
